<?php
/**
 * Add Bookmark page - dedicated page for adding new bookmarks
 */
...
// Get URL and title from query string (optional pre-fill)
$url = $_GET['url'] ?? '';
$title = $_GET['title'] ?? '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    if (!empty($submitUrl) && !empty($submitTitle)) {
        ...
        try {
            ...
            $success = true;
            // Redirect to bookmarks list after successful save
            header('Location: ' . $config['base_path'] . '/?success=1');
            exit;
        } catch (PDOException $e) {
...
<body>
    <?php render_nav($config, $isLoggedIn, 'add-bookmark', 'Add Bookmark'); ?>
            ...
            <form method="post" action="">
                ...
                <div class="form-group">
                    <label class="checkbox-label">
                        <input type="checkbox" id="private" name="private" <?= ($isEdit && !empty($existingBookmark['private'])) ? 'checked' : '' ?>>
